...
exports.getAssetsGeoJSON = getAssetsGeoJSON;
exports.createAsset = createAsset;
const db_1 = require("../config/db");
async function getAssetsGeoJSON(req, res) {
    try {
...
async function createAsset(req, res) {
    try {
        ...
        const result = await db_1.pool.query(`INSERT INTO assets (id, ward_id, asset_type, name, lbd_asset_id, status, location, attributes)
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         updated_at = now()
       RETURNING id, asset_type, status, version_id`, [id, ward_id || 1, asset_type, name, lbd_asset_id, status || 'active', longitude, latitude, JSON.stringify(attributes || {})]);
