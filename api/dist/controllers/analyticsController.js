...
exports.getCoverageGaps = getCoverageGaps;
exports.getSpatialGridHotspots = getSpatialGridHotspots;
...
async function getSummary(req, res) {
    try {
        ...
        // Asset count by type
        const assetCounts = await db_1.pool.query(`SELECT asset_type, COUNT(*) as count 
       FROM assets 
       WHERE ward_id = $1 
       GROUP BY asset_type`, [wardId]);
        // Issue count by category
        const categoryCounts = await db_1.pool.query(`SELECT category, COUNT(*) as count 
       FROM issues 
       WHERE ward_id = $1 
       GROUP BY category`, [wardId]);
        // Issue count by status
        const statusCounts = await db_1.pool.query(`SELECT status, COUNT(*) as count 
       FROM issues 
       WHERE ward_id = $1 
       GROUP BY status`, [wardId]);
        // Issue count by severity
        const severityCounts = await db_1.pool.query(`SELECT severity, COUNT(*) as count 
       FROM issues 
       WHERE ward_id = $1 
       GROUP BY severity`, [wardId]);
        return res.json({
            ward_id: wardId,
...
async function getCoverageGaps(req, res) {
    try {
        ...
        const radiusMeters = parseInt(distance_m) || 500;
        const wardId = ward_id || 1;
    ...
    `;
        const result = await db_1.pool.query(query, [wardId, radiusMeters]);
...
async function getSpatialGridHotspots(req, res) {
    try {
        const { ward_id } = req.query;
        const wardId = ward_id || 1;
    ...
    `;
        const result = await db_1.pool.query(query, [wardId]);
